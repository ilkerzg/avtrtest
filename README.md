...
使用 [Fal.ai](https://fal.ai) 的 TTS 服务，支持多种模型：
  - `kokoro` - 快速英文 TTS，多种语音可选
  - `xtts` - 支持中文和语音克隆的多语言 TTS
  - `f5-tts` - 高质量语音合成
  - `mars5-tts` - 深度语音克隆

**配置方法：**
1. 在 [fal.ai/dashboard/keys](https://fal.ai/dashboard/keys) 获取 API Key
2. 在 `config.yml` 中配置：
```yaml
FAL:
  FAL_API_KEY: "your-api-key"
  FAL_TTS_MODEL: "kokoro"  # 或 xtts, f5-tts, mars5-tts
  FAL_TTS_VOICE: "af_heart"  # kokoro 语音选项
```
3. 运行时指定 TTS 类型：
```bash
...
除了默认的 OpenAI 兼容 API，还支持使用 Fal.ai 的 LLM 服务：

**配置方法：**
1. 在 `config.yml` 中配置 LLM 模型：
```yaml
FAL:
  FAL_API_KEY: "your-api-key"
  FAL_LLM_MODEL: "google/gemini-2.5-flash"  # 或 anthropic/claude-3.5-sonnet, openai/gpt-4o
```
2. 设置环境变量启用 Fal LLM：
```bash
...
**支持的 LLM 模型：**
- `google/gemini-2.5-flash` - Google Gemini (推荐，快速)
