...
from src.config import get_doubao_appid, get_doubao_token, get_doubao_voice, get_fal_api_key, get_fal_tts_model, get_fal_tts_voice
...
###########################################################################################
class FalTTS(BaseTTS):
    """Fal.ai TTS implementation supporting multiple models (kokoro, xtts, f5-tts)"""
    
    def __init__(self, opt, parent):
        super().__init__(opt, parent)
        
        # 获取 fal.ai 配置
        self.api_key = get_fal_api_key()
        self.model = get_fal_tts_model()  # kokoro, xtts, f5-tts
        self.voice = get_fal_tts_voice()
        
        if not self.api_key:
            raise ValueError("FAL_API_KEY is required for FalTTS. Please set it in config.yml")
        ...
        try:
            ...
            raise ImportError("fal-client package is required. Install it with: pip install fal-client")
        
        logger.info(f"FalTTS initialized with model: {self.model}, voice: {self.voice}")
        logger.info(f"FalTTS API Key: {self.api_key[:10]}...")
        ...
        if self.model not in self.model_configs:
            logger.warning(f"Unknown model {self.model}, defaulting to kokoro")
            self.model = 'kokoro'
        
        self.config = self.model_configs[self.model]
        
        # 参考音频 URL (用于语音克隆模型)
        self.ref_audio_url = getattr(opt, 'REF_FILE', None)
        if self.config['requires_ref_audio'] and not self.ref_audio_url:
            logger.warning(f"Model {self.model} requires reference audio. Set REF_FILE in config.")
    ...
    def txt_to_audio(self, msg: tuple[str, dict]):
        ...
        try:
            ...
            # 构建请求参数
            input_params = self._build_input_params(text)
            ...
            logger.debug(f"FalTTS API call took: {end - start:.2f}s")
            
            # 处理音频结果
            self._process_audio_result(result, msg)
            
        except Exception as e:
            logger.exception(f"FalTTS txt_to_audio error: {e}")
            # 发送结束事件
            eventpoint = {'status': 'end', 'text': text}
            eventpoint.update(**textevent)
            self.parent.put_audio_frame(np.zeros(self.chunk, np.float32), eventpoint)

    def _build_input_params(self, text: str) -> dict:
        """根据模型构建输入参数"""
        if self.model == 'kokoro':
            ...
            params = {
                ...
                'audio_url': self.ref_audio_url,
                'sample_rate': 24000
            }
            if not self.ref_audio_url:
                # 使用默认参考音频
                params['audio_url'] = 'https://storage.googleapis.com/falserverless/model_tests/whisper/dinner_conversation.mp3'
            return params
        elif self.model == 'f5-tts':
            return {
                'gen_text': text,
                'model_type': 'F5-TTS',
                'ref_audio_url': self.ref_audio_url or 'https://storage.googleapis.com/falserverless/example_inputs/reference_audio.wav',
                'ref_text': '',  # 可选：参考音频的文本
                'remove_silence': True
            }
        elif self.model == 'mars5-tts':
            return {
                'text': text,
                'audio_url': self.ref_audio_url or 'https://storage.googleapis.com/falserverless/model_tests/mars5/female_speaker_1.flac',
                'deep_clone': True
            }
        else:
            return {'prompt': text}

    def _process_audio_result(self, result: dict, msg: tuple[str, dict]):
        """处理音频结果并发送到队列"""
        text, textevent = msg
        ...
        if 'audio' in result:
            ...
            if isinstance(result['audio_url'], dict):
                audio_url = result['audio_url'].get('url')
            else:
        ...
        logger.debug(f"FalTTS audio URL: {audio_url}")
        ...
        while streamlen >= self.chunk:
            ...
            current_frame = audio_array[idx:idx + self.chunk]
            self.parent.put_audio_frame(current_frame, eventpoint)
            streamlen -= self.chunk
            idx += self.chunk
