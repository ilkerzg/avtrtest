...
def get_fal_config() -> dict:
    ...
    """
    config = get_config()
    return config.get('FAL', {})


def get_fal_api_key() -> str:
    """获取 Fal.ai API Key"""
    return get_fal_config().get('FAL_API_KEY', '')


def get_fal_tts_model() -> str:
    """获取 Fal.ai TTS 模型 (kokoro, xtts, f5-tts, mars5-tts)"""
    return get_fal_config().get('FAL_TTS_MODEL', 'kokoro')


def get_fal_tts_voice() -> str:
    """获取 Fal.ai TTS 声音 (用于 kokoro)"""
    return get_fal_config().get('FAL_TTS_VOICE', 'af_heart')


def get_fal_llm_model() -> str:
    """获取 Fal.ai LLM 模型"""
    return get_fal_config().get('FAL_LLM_MODEL', 'google/gemini-2.5-flash')
